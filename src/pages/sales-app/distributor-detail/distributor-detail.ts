...
export class DistributorDetailPage {
    ...
    lead_detail()
    {
        ...
        this.dbService.onPostRequestDataFromApi({'dr_id':this.dr_id},'Distributor/lead_detail', this.dbService.rootUrlSfa).subscribe((result)=>{
            console.log(result);
    ...
    {
        this.dbService.onPostRequestDataFromApi({'dr_id':dr_id},'Checkin/customer_checkin', this.dbService.rootUrlSfa).subscribe((result)=>{
            console.log(result);
    ...
    {
        console.log(dr_id);
        this.dbService.onPostRequestDataFromApi({'dr_id':dr_id},'Distributor/customer_order', this.dbService.rootUrlSfa).subscribe((result)=>{
            console.log(result);
    ...
    check_user_right()
    {
        this.dbService.onPostRequestDataFromApi({data:'data'},'Distributor/check_user_right', this.dbService.rootUrlSfa)
        .subscribe((result)=>
        {
            console.log(result);
    ...
    edit_discount()
    {
        this.dbService.onPostRequestDataFromApi({"dr_id":this.dr_id},"dealerData/get_dealer_discount", this.dbService.rootUrlSfa)
        .subscribe(resp=>{
            console.log(resp);
    ...
    update_discount()
    {
        ...
        for (let index = 0; index < this.discount_list.length; index++) {
            if(this.discount_list[index].discount>100)
            {
                this.discount_list[index].discount = 0
                this.dbService.presentToast('Discount can not be greater than 100 % ')
                return;
            }

        }
        this.dbService.onPostRequestDataFromApi({"dr_id":this.dr_id,"discount_arr":this.discount_list},"dealerData/update_discount", this.dbService.rootUrlSfa)
        .subscribe(resp=>{
            console.log(resp);
            if(resp['msg'] == "success")
            {
    ...
    viewProfiePic(src)
    {
      this.modalCtrl.create(ViewProfilePage, {"Image": "http://phpstack-83335-1980946.cloudwaysapps.com/uploads/"+src}).present();
    }
    ionViewDidLeave()
    {
