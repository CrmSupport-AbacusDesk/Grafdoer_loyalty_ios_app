...
import { IonicPage, NavController, NavParams, ActionSheetController, LoadingController, AlertController, Loading, } from 'ionic-angular';
...
image:any='';
  ...
  fileChange(img)
 {
  ...
  genrateCompliant(){
    ...
    console.log("complaint data send",this.complaint_data.type);
    ...
    this.dbService.onPostRequestDataFromApi( {'complaint':this.complaint_data },'app_karigar/addComplaint', this.dbService.rootUrl).subscribe(result =>
      { 
        ...
        if (this.complaint_data.type =='Installation') {
          this.showSuccess("Success!", "New Installation request submitted successfully!");
        }
        if (this.complaint_data.type =='Service') {
          this.showSuccess("Success!", "New Service request submitted successfully!");
        }
  ...
  submit()
  {
    ...
    capturevideo()
    {
            let options: CaptureVideoOptions = { limit: 1, quality:0, duration:20 };
            this.mediaCapture.captureVideo(options)
            .then((videodata: MediaFile[]) => {
              console.log('media video', videodata);
    ...
    uploadVideo()
    {
      ...
      console.log('start');
     console.log('Video Url API Before',this.videoId);
     
      fileTransfer.upload(this.videoId,"https://apps.abacusdesk.com/grafdoer/dd_api/app/uploadVideos.php", options1)
      .then((data :any) => {
        console.log('Video Url API After',data);
        console.log('Video Url API After',this.videoId);
        var d = JSON.parse(data.response);
        console.log('Api Response',d);
        console.log("video data response",data.response);
        ...
        if(d.status == 'Success' ){
          ...
          if( d.status== 'Failed' ){
            this.loading.dismiss();
            this.showSuccess("Failed", "Uploading Failed!");

          }
          if( d.status == 'Wrong' ){
            this.loading.dismiss();
            this.showSuccess("Alert", "Somthing went Wrong!");
